<?php
    /**
     * @package POWr Count Up Timer
     * @version 1.6.4
     */
    /*
    Plugin Name: POWr Count Up Timer
    Plugin URI: https://www.powr.io/plugins/count-up-timer
    Description: Count up from any time, date, or number.  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////Create Count Up Timer widget/////////////////////////////////
    class Powr_Count_Up_Timer extends WP_Widget{
      public $plugin;
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_count_up_timer',
                             __( 'POWr Count Up Timer' ),
                             array( 'description' => __( 'Count Up Timer by POWr.io') )
        );
      ...
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-count-up-timer' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Count Up Timer.
        </p>
        <p>
    ...
    function register_powr_count_up_timer() {
      register_widget( 'Powr_Count_Up_Timer' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_count_up_timer' );
    //Create short codes for adding plugins anywhere:
    function powr_count_up_timer_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-count-up-timer' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-count-up-timer' label='$label'></div>";
      }else{
      	"<div class='powr-count-up-timer'></div>";
      }
    }
    add_shortcode( 'powr-count-up-timer', 'powr_count_up_timer_shortcode' );
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_count_up_timer_menu' );
    function powr_count_up_timer_menu() {
      add_menu_page( 'POWr Count Up Timer', 'POWr Count Up Timer', 'manage_options', 'powr-count-up-timer-settings', 'powr_count_up_timer_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_count_up_timer_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/count-up-timer';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/count-up-timer';
      }
    ...
    register_activation_hook(__FILE__, 'powr_count_up_timer_plugin_activate');
      add_action('admin_init', 'powr_count_up_timer_plugin_redirect');

      function powr_count_up_timer_plugin_activate() {
      add_option('powr_count_up_timer_plugin_do_activation_redirect', true);
      }
      ...
      function powr_count_up_timer_plugin_redirect() {
      if (get_option('powr_count_up_timer_plugin_do_activation_redirect', false)) {
          delete_option('powr_count_up_timer_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-count-up-timer-settings&' );
          }
       }
      }
